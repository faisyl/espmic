...
func TestInteropNoLeak100Streams(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		port, err := r.Bind(context.Background(), streamID, DefaultPayloadType, 60*time.Millisecond, 0)
		if err != nil {
...
func TestInteropTwoDevices(t *testing.T) {
	r := NewReceiver(metrics.New())
	_, err := r.Bind(context.Background(), "s1", DefaultPayloadType, 60*time.Millisecond, 0)
	if err != nil {
		t.Fatalf("bind s1: %v", err)
	}
	_, err = r.Bind(context.Background(), "s2", DefaultPayloadType, 60*time.Millisecond, 0)
	if err != nil {
...
func TestInteropMalformedNoCrash(t *testing.T) {
	r := NewReceiver(metrics.New())
	port, err := r.Bind(context.Background(), "s1", DefaultPayloadType, 60*time.Millisecond, 0)
	if err != nil {
		t.Fatalf("bind: %v", err)
	}
	defer r.CloseStream("s1")
